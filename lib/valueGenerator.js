...
class Any {
    ...
    generate() {
        ...
        if (Hoek.reach(schemaDescription, 'examples')) {
            return Helpers.pickRandomFromArray(schemaDescription.examples);
        }
...
class StringExample extends Any {
    ...
    _generate(rules) {
        ...
        const specials = {
            ...
            token: () => new RandExp(/[a-zA-Z0-9_]+/).gen(),
            hex: () => new RandExp(/^[a-f0-9]+$/i).gen(),
            creditCard: () => {
            ...
            pattern: () => {
                ...
                const pattern = Hoek.reach(rules, 'pattern.options.invert') ? /[a-f]{3}/ : rules.pattern.regex;
        ...
        if (rules.length) {
            ...
            if (stringResult.length < minLength) {
                ...
                while (stringResult.length < minLength) {
